...
class QueryControllers{
  ...
  async handleCreateQuery(request: Request, response: Response) {
      const {  numero,fecha,dueno,doctor,queryname, patientId, reason, diagnosis  } = request.body;
  ...
  async handleDeleteQuery(request: Request, response: Response) {
    ...
    try {
      ...
      response.render("Query/messageQuery", {
        message: `Error al eliminar consulta: ${err.message}`
      });
    }
  } 
  ...
  async handleUpdateQuery(request: Request, response: Response) {
    const { idQuery,numero,fecha,dueno,doctor, queryname, patientId, reason, diagnosis } = request.body;
    const updateQueryService = new QueryService();

    try {
      await updateQueryService.update({ idQuery, numero,fecha,dueno,doctor,queryname, patientId, reason, diagnosis }).then(() => {
        response.render("Query/messageQuery", {
